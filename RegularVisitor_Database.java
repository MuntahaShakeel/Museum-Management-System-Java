...
public class RegularVisitor_Database {
    private String name;
    private int id;
    private String dateOfBirth;
    private Connect c;

    public RegularVisitor_Database(String name, int id, String dateOfBirth) {
        this.name = name;
        this.id = id;
    ...
    public void insertVisitor() {
        String query = "INSERT INTO RegularVisitor (Name, ID, [Date of Birth]) VALUES (?, ?, ?)";
        try {
    ...
    public void displayVisitors() {
        ...
        if (rs == null) {
            System.out.println("No data found in the RegularVisitor table or failed to fetch data.");
            return;
        }
        ...
        try {
            ...
            if (!hasData) {
                System.out.println("The RegularVisitor table is empty.");
            }
        } catch (SQLException e) {
    ...
    public void deleteVisitorById(int visitorId) {
        String query = "DELETE FROM RegularVisitor WHERE ID = ?";
        try {
            Connection conn = c.getConnection();
            if (conn != null) {
                PreparedStatement ps = conn.prepareStatement(query);
    ...
    private void shiftVisitorsUp() {
        String query = "UPDATE RegularVisitor SET ID = ID - 1 WHERE ID > ?";
        try {
            Connection conn = c.getConnection();
            if (conn != null) {
                PreparedStatement ps = conn.prepareStatement(query);
    ...
    public void updateVisitor(int visitorId, String newName, int newId, String newDateOfBirth) {
        String query = "UPDATE RegularVisitor SET Name = ?, ID = ?, [Date of Birth] = ? WHERE ID = ?";
        try {
            Connection conn = c.getConnection();
            if (conn != null) {
                PreparedStatement ps = conn.prepareStatement(query);
    ...
    public ResultSet searchVisitorById(int visitorId) {
        String query = "SELECT * FROM RegularVisitor WHERE ID = ?";
        try {
            Connection conn = c.getConnection();
            if (conn != null) {
                PreparedStatement ps = conn.prepareStatement(query);
